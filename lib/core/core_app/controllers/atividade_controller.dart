...
class AtividadeController extends GetxController {
  ...
  Future<void> _buscarAtividadeEmAndamento() async {
    try {
      ...
      if (atividade != null) {
        ...
        AppLogger.d(
            '✅ Atividade em andamento encontrada (ID: ${atividade.uuid}), etapa inicial: APR',
            tag: 'AtividadeController');
      } else {
  ...
  Future<void> iniciarAtividade(AtividadeTableDto atividade) async {
    try {
      AppLogger.d('⚙️ Iniciando atividade ID: ${atividade.uuid}',
          tag: 'AtividadeController');
      ...
      final index = atividades.indexWhere((a) => a.uuid == atividade.uuid);
      if (index != -1) {
        // atividades[index] = atividade.copyWith(status: StatusAtividade.emAndamento);
        atividades.refresh();
  ...
  Future<EtapaAtividade?> _proximaEtapa(
      AtividadeTableDto atividade, EtapaAtividade etapaAtual) async {
  ...
  Future<void> executarAtividade(AtividadeTableDto atividade) async {
    ...
    AppLogger.d(
        '➡️ Executando etapa: $etapa para atividade ID: ${atividade.uuid}',
        tag: 'AtividadeController');
