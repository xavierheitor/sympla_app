...
class AprQuestionTableSyncImpl
  ...
  Future<List<AprQuestionTableDto>> buscarDaApi() async {
    try {
      ...
      AppLogger.e(
        '[apr_question_table_sync_impl - buscarDaApi] ${erro.mensagem}',
        tag: 'AprQuestionTableSyncImpl',
        error: e,
        stackTrace: s,
      );
      return [];
  ...
  Future<bool> estaVazio(String entidade) async {
    try {
      return await aprDao.countQuestoes() == 0;
    } catch (e, s) {
      final erro = ErrorHandler.tratar(e, s);
      AppLogger.e(
        '[apr_question_table_sync_impl - estaVazio] ${erro.mensagem}',
        tag: 'AprQuestionTableSyncImpl',
        error: e,
        stackTrace: s,
      );
  ...
  @override
  Future<void> sincronizarComBanco(List<AprQuestionTableDto> itens) async {
    try {
      final data = await buscarDaApi();
      final itens = data.map((e) => e.toCompanion()).toList();
      await aprDao.sincronizarPerguntas(itens);
    } catch (e, s) {
      final erro = ErrorHandler.tratar(e, s);
      AppLogger.e(
        '[apr_question_table_sync_impl - sincronizarComBanco] ${erro.mensagem}',
        tag: 'AprQuestionTableSyncImpl',
        error: e,
        stackTrace: s,
      );
