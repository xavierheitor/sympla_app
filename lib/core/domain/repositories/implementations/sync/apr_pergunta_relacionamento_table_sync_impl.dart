...
class AprPerguntaRelacionamentoTableSyncImpl
  ...
  Future<List<AprPerguntaRelacionamentoTableDto>> buscarDaApi() async {
    try {
      ...
      AppLogger.e(
          '[apr_pergunta_relacionamento_table_sync_impl - buscarDaApi] ${erro.mensagem}',
          tag: 'AprPerguntaRelacionamentoTableSyncImpl',
          error: e,
          stackTrace: s);
      return [];
    }
  ...
  Future<bool> estaVazio(String entidade) async {
    try {
      return await aprDao.countRelacionamentos() == 0;
    } catch (e, s) {
      final erro = ErrorHandler.tratar(e, s);
      AppLogger.e(
          '[apr_pergunta_relacionamento_table_sync_impl - estaVazio] ${erro.mensagem}',
          tag: 'AprPerguntaRelacionamentoTableSyncImpl',
          error: e,
          stackTrace: s);
      return true;
  ...
  Future<void> sincronizarComBanco(
      List<AprPerguntaRelacionamentoTableDto> itens) async {
    try {
      final data = await buscarDaApi();
      final itens = data.map((e) => e.toCompanion()).toList();
      await aprDao.sincronizarRelacionamentos(itens);
    } catch (e, s) {
      final erro = ErrorHandler.tratar(e, s);
      AppLogger.e(
          '[apr_pergunta_relacionamento_table_sync_impl - sincronizarComBanco] ${erro.mensagem}',
          tag: 'AprPerguntaRelacionamentoTableSyncImpl',
          error: e,
          stackTrace: s);
    }
